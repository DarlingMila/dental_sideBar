...
function checkValue(e) {
  ...
  */
  if (!Boolean(value) && parentNumber == numberOfChildren - 1) main.lastElementChild.remove();
...
function addToOptions(serviceTitle, steps, meds, blockNumber) {
  ...
  const block = main.querySelector(`.block_${blockNumber}`);
...
function showSelectedPositions() {
  ...
  const mainHeader = createNewItem(
    "h3",
    null,
    "Выбранные позиции:",
    ["submitScreen__header"]
  );
  modalBody.appendChild(mainHeader);
...
function getTextWarning(allServicesArray) {
  ...
  if (length > 0) {
    ...
    for (let i = 0; i < length; i++) {
      message += `NEWLINE- "${array[i][0]}" повторяется ${array[i][1]} раз(а)`;
    }
  }
